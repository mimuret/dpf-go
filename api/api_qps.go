...
type ApiGetQpListRequest struct {
	ctx        context.Context
	ApiService *QpsAPIService
	contractId string
}

func (r ApiGetQpListRequest) Execute() (*GetQpsHistories, *http.Response, error) {
	return r.ApiService.GetQpListExecute(r)
}

/*
GetQpList 月別のQPSの一覧取得
...
ゾーンおよびLBドメインのクエリ数を元に集計したDPF契約の月別のQPSが取得できます。
 ...
 @param contractId ID
 @return ApiGetQpListRequest
*/
func (a *QpsAPIService) GetQpList(ctx context.Context, contractId string) ApiGetQpListRequest {
	return ApiGetQpListRequest{
		ApiService: a,
		ctx:        ctx,
...
func (a *QpsAPIService) GetQpListExecute(r ApiGetQpListRequest) (*GetQpsHistories, *http.Response, error) {
	ctx, span := a.client.tracer.Start(r.ctx, "QpsAPIService.GetQpListExecute")
	defer span.End()
	r.ctx = ctx
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "QpsAPIService.GetQpList")
	if err != nil {
